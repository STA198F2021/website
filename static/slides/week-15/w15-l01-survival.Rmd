...
It is important to distinguish between calendar time and participant time.
...
Note: because there was only 1 bedridden patient, the median survival time is the survival time of that patient in ph.ecog=3, and there is no confidence interval provided.
...
coxph(Surv(time, status) ~ as.factor(ph.ecog)+wt.loss+age, data = lung) %>%
...
So our HR = 2.5 implies that around 2.5 times as many people who are bedridden are dying as those who are asymptomatic, at any given time.
