...
While we could use the Central Limit Theorem here, we actually have an even better option in R -- we can carry out this test based on the (exact) binomial distribution. With the binomial distribution, we don't have to worry about the sample size or rules of thumb for our test to be accurate.
...
```
---
Note the test statistic given here, `X-squared`, refers to a $\chi^2$ test, and the particular $\chi^2$ test here is obtaining by taking $z^2$ from our prior normal distribution test derivation. It turns out that the square of a N(0,1) random variable follows a $\chi^2$ distribution with one degree of freedom.
