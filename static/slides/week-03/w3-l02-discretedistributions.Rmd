...
In IVF, counting starts on the day the egg is fertilized (corresponding to around day 14 of a regular pregnancy if the sperm meets the egg right away after it is released!).
]
.pull-right[
```{r out.width="100%", echo=FALSE}
knitr::include_graphics("embryo.jpg")
```
]
...
- Probability of having at least one baby is $P(Y \geq 1)$, which we want to be large

- Probability of having triplets, quadruplets, or a higher-order birth is $P(Y>2)$, which we want to be small
...
For the last row of the table on the prior slide, $n=5$ and $\pi=0.35$. The probability of at least one baby, $P(Y \geq 1)=1-P(Y=0)$ can be calculated using in R the code *1-dbinom(0,5,0.35)*, which yields the value `r 1-dbinom(0,5,0.35)`.
