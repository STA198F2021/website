...
---
Ahh, there were 18 missing values of the variable `bmiclass`!  First, let's check to be sure we didn't code that new variable incorrectly by seeing if the source varialbe, BMI, also has some missing values.
...
OK great, so the problem was with the source variable. We can just drop those few NA's when we take the mean, so that we get the mean of the observed values.
]
]
